...
public class PasswordResetIntegrationTest {
    ...
    void testPasswordResetFlow_Success() throws Exception {
        ...
        assertTrue(passwordEncoder.matches("NewPassword123!", updatedUser.getPassword()), "Password should be updated");
        assertNotNull(updatedUser.getLastPasswordChangeAt());

        Otp updatedOtp = otpRepository.findByEmailAndOtpType(testUser.getEmail(), OtpType.FORGOT_PASSWORD).orElseThrow();
