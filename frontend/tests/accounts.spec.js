...
test.describe.serial('Accounts - OpenAccountForm flows (focused)', () => {
  ...
  test.beforeEach(async () => {
    ...
    await expect(sharedPage.getByRole('button', { name: /Open New Account/i }).first()).toBeVisible({ timeout: 20_000 });
  });
  ...
  test('Acceptance: Open New Account option is available and Savings option exists', async () => {
    await sharedPage.getByRole('button', { name: /Open New Account/i }).first().click();
    await sharedPage.getByLabel('Account Type').click();
    // Ensure both options are available
    await expect(sharedPage.getByRole('option', { name: 'Chequing' })).toBeVisible();
    await expect(sharedPage.getByRole('option', { name: 'Savings' })).toBeVisible();
  ...
  test('Acceptance: Required fields are indicated and Next is disabled until valid', async () => {
    ...
    await fillRequiredFields(sharedPage, { accountName: 'ReqCheck', skipOpenClick: true });
  ...
  test('Scenario: Successfully open a chequing account and it appears in Accounts list', async () => {
    ...
    await fillRequiredFields(sharedPage, { accountName: acctName, accountType: 'Chequing' });

    // proceed through terms and submit
    await sharedPage.getByRole('button', { name: /Next/i }).click();
    await sharedPage.getByLabel('I have read and accept the Terms and Conditions.').check();
    await sharedPage.getByRole('button', { name: /Next/i }).click();

    await expect(sharedPage.getByRole('heading', { name: 'Review & Submit' })).toBeVisible();
    await sharedPage.getByRole('button', { name: /Submit/i }).click();

    // dialog should close
    await expect(sharedPage.getByRole('heading', { name: 'Open a New Account' })).toHaveCount(0, { timeout: 10_000 });

    // the account card with the name should appear
    await expect(sharedPage.getByText(acctName).first()).toBeVisible({ timeout: 10_000 });
  });
  ...
  test('Scenario: Missing/invalid information prevents submission and shows validation', async () => {
    ...
    await expect(sharedPage.getByRole('heading', { name: 'Open a New Account' })).toBeVisible();
  ...
  test('Acceptance: Can create two accounts with different names and numbers are unique', async () => {
    ...
    await sharedPage.getByRole('button', { name: /Next/i }).click();
    await sharedPage.getByRole('button', { name: /Submit/i }).click();
    await expect(sharedPage.getByText(name1).first()).toBeVisible({ timeout: 10_000 });

    // capture its account number suffix
    const card1 = sharedPage.getByText(name1).first().locator('..').locator('..');
    ...
    await sharedPage.getByRole('button', { name: /Next/i }).click();
    await sharedPage.getByRole('button', { name: /Submit/i }).click();
    await expect(sharedPage.getByText(name2).first()).toBeVisible({ timeout: 10_000 });

    const card2 = sharedPage.getByText(name2).first().locator('..').locator('..');
