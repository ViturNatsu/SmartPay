...
import { getPayeesByUserId } from "@/api/payees/payeeApi";
        ...
        <Stack key={label} direction="row" alignItems="center" spacing={1}>
          <Box
            sx={{
              ...
              height: 28,
              borderRadius: "50%",
              bgcolor: i < current ? "#0f7490" : i === current ? "#0f7490" : "#E5E7EB",
...
function PayeeRow({ payee, selected, onSelect }) {
  const initials =
    `${payee.recipientFirstName?.[0] ?? ""}${payee.recipientLastName?.[0] ?? ""}`.toUpperCase();
      ...
      <Stack direction="row" spacing={1.5} alignItems="center">
        ...
        <Box>
          <Typography sx={{ fontWeight: 700, fontSize: 14 }}>
            {payee.recipientFirstName} {payee.recipientLastName}
          </Typography>
          <Typography sx={{ color: "#64748b", fontSize: 12 }}>
            {payee.recipientEmail}
          </Typography>
        </Box>
      </Stack>
      <Box
...
export function MakeAPayment() {
  ...
  useEffect(() => {
    ...
    const userId = Number(tokenClaims.userId);

    Promise.all([getPayeesByUserId(userId), getWalletByUserId(userId)])
  ...
  // ── Step 1 helpers ──────────────────────────────────────────────
  const handleSelectPayee = (payee) => setSelectedPayee(payee);
        ...
        <Box sx={{ maxWidth: 720, mx: "auto" }}>
          ...
          {step === 0 && (
            <>
              ...
              <Card
                elevation={0}
                sx={{ borderRadius: "18px", border: "1px solid #E2E8F0", overflow: "hidden" }}
              >
                <Box sx={{ p: "20px 24px", borderBottom: "1px solid #E2E8F0" }}>
                  ...
                  <Typography sx={{ fontSize: 12, color: "#94a3b8", mt: 0.5 }}>
                    Scroll to see all saved payees
                  </Typography>
                </Box>
          ...
          {step === 1 && (
            <>
              <Typography variant="h5" sx={{ fontWeight: 800, mb: 0.5 }}>
                Send to {selectedPayee.recipientFirstName} {selectedPayee.recipientLastName}
              </Typography>
              <Typography sx={{ color: "#64748b", fontSize: 14, mb: 3 }}>
              ...
              <Stack direction={{ xs: "column", sm: "row" }} spacing={1.5} mb={2.5}>
                {[
                  ...
                  },
                  {
                    ...
                    value: `${selectedPayee.recipientFirstName} ${selectedPayee.recipientLastName}`,
                  },
                ].map(({ label, value }) => (
                  <Box
                    key={label}
                  ...
                  >
                    <Typography sx={{ color: "#64748b", fontSize: 12, fontWeight: 700, mb: 0.75 }}>
                      {label}
                    </Typography>
              ...
              <Card
                elevation={0}
                sx={{ borderRadius: "18px", border: "1px solid #E2E8F0", p: "24px" }}
              >
                <Stack spacing={2.5}>
                  <Box>
                    <TextField
                  ...
                  <Box>
                    <TextField
                      ...
                      error={!!memoError}
                      helperText={
                        memoError || `${memo.length}/100 characters — letters, numbers, and spaces only`
                      }
                      sx={{ "& .MuiOutlinedInput-root": { borderRadius: "12px" } }}
                    />
                  </Box>
          ...
          {step === 3 && (
            ...
            >
              ...
              <Typography sx={{ color: "#64748b", fontSize: 14, mb: 3.5, lineHeight: 1.6 }}>
                {formattedAmount} has been sent to {selectedPayee.recipientFirstName}{" "}
                {selectedPayee.recipientLastName} from your SmartPay wallet.
              </Typography>
              <Button
                variant="contained"
